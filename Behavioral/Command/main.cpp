...
* operation can be stored internally (as in amount of money involved in a transaction).
*
...
*  - command: it causes the chyange of the object, but does not have a return value (much like a set operation), causing a 
